...
const Orders = () => {
    ...
    const fetchOrders = async () => {
        setOrders(dummyOrders); // ✅ this is your placeholder, not modified  
    };
                ...
                {orders.map((order, index) => (
                    <div key={index} className="flex flex-col md:items-center md:flex-row gap-5 justify-between p-5 max-w-4xl rounded-md border border-gray-300">
                        <div className="flex gap-5 max-w-80">
                            <img className="w-12 h-12 object-cover" src={assets.box_icon} alt="boxIcon" /> {/* ✅ fixed */}
                            <div>
                                {order.items.map((item, i) => ( // ✅ changed inner index to `i`
                        ...
                        <p className="font-medium my-auto text-lg">{currency}{order.amount}</p>
